  ...
  */
	...
	for (let i = 0; i < alphabet.length; ++i) {
		...
		if (s1.includes(alphabet[i]) || s2.includes(alphabet[i])) {
			...
			const howManyTimesCharAppearsInS1 = [...s1].filter(
				(v) => v === alphabet[i]
			).length;
			const howManyTimesCharAppearsInS2 = [...s2].filter(
				(v) => v === alphabet[i]
			).length;
			// if total times character appears in either string is > 1, proceed with next steps
			if (
				howManyTimesCharAppearsInS1 > 1 ||
				howManyTimesCharAppearsInS2 > 1
			) {
				// if character appears identical number of times in both strings
				if (
					howManyTimesCharAppearsInS1 === howManyTimesCharAppearsInS2
				) {
					temp += `=:`;
				...
				temp += alphabet[i].repeat(
					Math.max(
						howManyTimesCharAppearsInS1,
						howManyTimesCharAppearsInS2
					)
				);
