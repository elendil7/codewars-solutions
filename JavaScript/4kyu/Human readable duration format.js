function formatDuration(secs) {
	...
	for (let i = 0; i < arr.length; ++i) {
		...
		if (arr[i] > 0) {
			...
			finalArr.push(
				`${arr[i]} ${currentTimeType[i]}${arr[i] > 1 ? `s` : ``}`
			);
		}
	}
	...
	if (finalArr.length > 1) {
		...
		return [
			...[...finalArr.join`, `].reverse().join``.replace(/,/, `dna `),
		].reverse().join``;
	} else {
...
formatDuration = (
	s,
	z = `year,day,hour,minute,second`.split`,`,
	e = 31536000,
	f = 86400,
	g = 3600,
	h = 60
	...
	return ((a) =>
		a[1]
			? [
					...[...a.join`, `].reverse().join``.replace(/,/, `dna `),
			  ].reverse().join``
			: a.join`, `)(
		y
			.map((v, i) =>
				v > 0 ? `${y[i]} ${z[i]}${y[i] > 1 ? `s` : ``}` : 0
			)
			.filter((v) => v)
	);
};
