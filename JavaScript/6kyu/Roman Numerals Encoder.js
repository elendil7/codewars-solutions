...
solution = (n, z = [1000, 900, 500, 400, 100, 90], r = []) => {
	while (n > 0) {
		if (n >= z[0])
			r.push(`M`.repeat(~~(n / z[0]))), (n -= ~~(n / z[0]) * z[0]);
		else if (n >= z[1])
			r.push(`CM`.repeat(~~(n / z[1]))), (n -= ~~(n / z[1]) * z[1]);
		else if (n >= z[2])
			r.push(`D`.repeat(~~(n / z[2]))), (n -= ~~(n / z[2]) * z[2]);
		else if (n >= z[3])
			r.push(`CD`.repeat(~~(n / z[3]))), (n -= ~~(n / z[3]) * z[3]);
		else if (n >= z[4])
			r.push(`C`.repeat(~~(n / z[4]))), (n -= ~~(n / z[4]) * z[4]);
		else if (n >= 90)
			r.push(`XC`.repeat(~~(n / 90))), (n -= ~~(n / 90) * 90);
		else if (n >= 50)
			r.push(`L`.repeat(~~(n / 50))), (n -= ~~(n / 50) * 50);
		else if (n >= 40)
			r.push(`XL`.repeat(~~(n / 40))), (n -= ~~(n / 40) * 40);
