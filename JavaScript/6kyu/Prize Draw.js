function rank(st, we, n) {
	...
	let ranks = names.map((v) =>
		[...v]
			.map((x) => z.indexOf(x[0].toLowerCase()))
			.reduce((a, b) => a + b, 0)
	);
		...
		z
			.map((v, i) => [
				...
				([...v].reduce(
					(a, b) =>
						a +
						`ᅟabcdefghijklmnopqrstuvwxyz`.indexOf(
							b[0].toLowerCase()
						),
					0
				) +
...
rank = (ᅟ, ॱ, ᅠ, ㅤ = ᅟ.split`,`) =>
	((ㅤ) =>
		...
		ㅤ
			...
			.sort((ᅟ, ᅠ) =>
				ᅟ[1] == ᅠ[1] ? ᅟ[0].localeCompare(ᅠ[0]) : ᅠ[1] - ᅟ[1]
			)
	);
