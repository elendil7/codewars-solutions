function stat(str) {
	...
	return `Range: ${convertToHours(range)} Average: ${convertToHours(
		average
	)} Median: ${convertToHours(median)}`;
}
...
stat = (
	...
	x = s
		.split(/, |,/)
		.map((v) => ((f) => +f[0] * 3600 + +f[1] * 60 + +f[2])(v.split`|`))
		.sort((a, b) => a - b),
	l = x.length,
	y = ~~(l / 2),
	r = Math.max(...x) - Math.min(...x),
	a = x.reduce((a, b) => a + b) / l,
	m = l % 2 ? x[y] : (x[y - 1] + x[y]) / 2
) => (!s ? `` : `Range: ${c(r)} Average: ${c(a)} Median: ${c(m)}`);
